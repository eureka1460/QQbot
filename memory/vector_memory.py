...
class VectorMemory:
    ...
    def _collection(self, collection_id: int, prefix: str = "group"):
        return self._client.get_or_create_collection(
            name=f"{prefix}_{collection_id}",
            metadata={"hnsw:space": "cosine"},
        )

    def store(self, group_id: int, user_id: Optional[int], content: str, role: str, prefix: str = "group") -> None:
        if not self._ready or not content or not content.strip():
            return
        col = self._collection(group_id, prefix)
    ...
    def clear(self, group_id: int, prefix: str = "group") -> bool:
        """Delete all stored messages for a group. Returns False if not ready."""
        if not self._ready:
            return False
        try:
            self._client.delete_collection(f"{prefix}_{group_id}")
            return True
        except Exception as exc:
            print(f"[Memory] Failed to clear group {group_id}: {exc}")
            return False

    def search(self, group_id: int, query: str, prefix: str = "group") -> str:
