...
_GROUP_CTX = (
    ...
    "\nprompt 中由系统附加的 [本轮运行时上下文] 是可信上下文，用来说明当前时间、关系模式、用户档案、长期记忆和群聊状态。"
...
_PRIVATE_CTX = (
    "\n\n[当前处于私聊中]"
    "\nprompt 中由系统附加的 [本轮运行时上下文] 是可信上下文，用来说明当前时间、关系模式、用户档案和长期记忆。"
)
_GROUP_RELATION_CTX = (
    "\n\n[群聊关系模式]"
    "\n每轮实际关系以 [本轮运行时上下文] 中的当前模式为准。"
    "\n- 当前模式为 master 时：当前用户是主人/超级用户，可以使用主人称呼，保持亲近、撒娇、信任的态度。"
    "\n- 当前模式为 guardian 时：当前用户不是主人，保持礼貌可爱但有距离感，不主动撒娇，不称呼对方为主人。"
    "\n如需在群聊中 @ 当前用户，使用 [CQ:at,qq=<当前用户QQ>]；实际 QQ 号以运行时上下文为准。"
)
...
class PersonaEngine:
    ...
    def _build_group_system_role(self) -> str:
        return self._stable_group_role(roles.get_Murasame_goshujin_role) + _GROUP_RELATION_CTX
    ...
    def _stable_group_role(self, role_builder: Callable[[int, int], str]) -> str:
        ...
        role_text = _RELATION_BLOCK_RE.sub("\n", role_text)
