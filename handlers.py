...
async def handler_init(interfaces):
    ...
    try:
        from memory.user_profile import ProfileManager
        profile_manager = ProfileManager()
        print(f"[Profile] Profile manager initialized")
    except Exception as exc:
        print(f"[Profile] Failed to initialize profile manager: {exc}")

    ban_manager = None
    try:
        from memory.banlist import BanManager
        ban_manager = BanManager()
        print(f"[Ban] Ban manager initialized")
    except Exception as exc:
        print(f"[Ban] Failed to initialize ban manager: {exc}")
    ...
    try:
        ...
        print(f"[Session] Session store initialized")
    except Exception as exc:
        print(f"[Session] Failed to initialize session store: {exc}")
